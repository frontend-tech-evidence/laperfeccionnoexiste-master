 ...
 */
function calcularCostoImplementacion(costoActivacion, costoMigracion, costoCapacitacion, isPagoImplementacionMensual = false, hasMigracionChecked = true, hasCapacitacionChecked = true) {
...
function calcularCostoCapacitacion(cantidadUsuariosRequeridos, usuariosGratisDeCapacitacion, costoCapacitacionBase, costoCapacitacionUsuarioExtra) {
    if (cantidadUsuariosRequeridos > usuariosGratisDeCapacitacion) {
        let cantidadUsuariosExtras = cantidadUsuariosRequeridos - usuariosGratisDeCapacitacion;
        const costoUsuariosExtrasCapacitaciones = cantidadUsuariosExtras * costoCapacitacionUsuarioExtra;
        return costoCapacitacionBase + costoUsuariosExtrasCapacitaciones;
    }
    return costoCapacitacionBase;
}
function calcularCostoMembresia(costoBaseMensual, isPagoMensualidadMensual = false) {
    const numeroMeses = 12;
...
function calcularCostoTimbres(timbresRequeridos, timbresGratisIncluidos, costoTimbreExtra) {
    let costoTotal = 0;
...
function calcularCostoFacturacionAnual(costoBaseMensual, costoUsuarios) {
...
function calcularCostoUsuario(cantidadUsuariosRequeridos, cantidadUsuariosGratisIncluidos, costoUsuarioExtra, hasPrecioUsuarioExtraVariable = false, costoUsuarioExtraDespuesDeLimite, cantidadDeUsuariosAntesDelDescuento) {
    if (hasPrecioUsuarioExtraVariable) {
        if (cantidadUsuariosRequeridos > cantidadDeUsuariosAntesDelDescuento) {
            const cantidadAntesDelRango = cantidadDeUsuariosAntesDelDescuento -
                cantidadUsuariosGratisIncluidos;
            const cantidadDespuesDelRango = cantidadUsuariosRequeridos - cantidadDeUsuariosAntesDelDescuento;
    ...
    const costo = Math.abs((cantidadUsuariosGratisIncluidos - cantidadUsuariosRequeridos) *
        costoUsuarioExtra);
    return costo;
}
function calcularCostoPrimerAno(costoBase, isImplementacionMensual, isPagoMensualidadMensual, costoImplementacion, costoMembresia, costoTimbres, costoUsuarios) {
    if (!isImplementacionMensual) {
        costoMembresia = costoMembresia - costoBase;
    }
    let costoTotalFinal = Math.floor(costoImplementacion + costoMembresia + costoTimbres + costoUsuarios);
    if (!isPagoMensualidadMensual) {
...
export function getAllCostosPaquetes(atributosDeCostosDinamicosPaquetes, ...paquetes) {
    ...
    for (const paquete of paquetes) {
        const costoCapacitacion = calcularCostoCapacitacion(atributosDeCostosDinamicosPaquetes.usuariosRequeridos, paquete.usuariosGratisDeCapacitacion, paquete.costoCapacitacion, paquete.costoCapacitacionUsuarioExtra);
        const costoImplementacion = calcularCostoImplementacion(paquete.costoActivacion, paquete.costoMigracion, costoCapacitacion, atributosDeCostosDinamicosPaquetes.isPagoImplementacionMensual, paquete.hasMigracionChecked, paquete.hasCapacitacionChecked);
        const costoMembresia = calcularCostoMembresia(paquete.costoBaseMensual, atributosDeCostosDinamicosPaquetes.isPagoMensualidadMensual);
